...
pub fn addZigStart(
    ...
    const lib = builder.addStaticLibrary(.{
        .name = "start",
        .root_source_file = builder.path(relpath("src" ++ std.fs.path.sep_str ++ "start.zig")),
        .target = target,
        .optimize = optimize,
    });
...
pub fn addLibc(builder: *std.Build, opt: ZigLibcOptions) *CompileStep {
    ...
    const index = builder.path(relpath("src" ++ std.fs.path.sep_str ++ "lib.zig"));
    ...
    if (include_cstd) {
        lib.addCSourceFile(.{ .file = builder.path(relpath("src" ++ std.fs.path.sep_str ++ "printf.c")), .flags = &c_flags });
        lib.addCSourceFile(.{ .file = builder.path(relpath("src" ++ std.fs.path.sep_str ++ "scanf.c")), .flags = &c_flags });
        if (opt.target.result.os.tag == .linux) {
            lib.addAssemblyFile(builder.path(relpath("src/linux/jmp.s")));
        }
    }
    ...
    if (include_posix) {
        lib.addCSourceFile(.{ .file = builder.path(relpath("src" ++ std.fs.path.sep_str ++ "posix.c")), .flags = &c_flags });
    }
    ...
    if (include_cstd or include_posix) {
        lib.addIncludePath(builder.path(relpath("inc" ++ std.fs.path.sep_str ++ "libc")));
        lib.addIncludePath(builder.path(relpath("inc" ++ std.fs.path.sep_str ++ "posix")));
    }
    ...
    if (include_gnu) {
        lib.addIncludePath(builder.path(relpath("inc" ++ std.fs.path.sep_str ++ "gnu")));
    }
    return lib;
}
